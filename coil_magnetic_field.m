...
function base_points = generateCoilBasePoints(r_inner, L_straight, arc_r, arc_cx, arc_cy, ...
    ...
    y_line = linspace(0, L_straight/2, n_line + 1);
    y_line = y_line(1:end-1);  % 排除终点，避免与第一段圆弧起点重复
    ...
    for arc_idx = 1:3
        ...
        if arc_idx < 3
            % 非最后一段圆弧：排除终点，避免与下一段圆弧起点重复
            theta_seq = linspace(deg2rad(arc_start(arc_idx)), deg2rad(arc_end(arc_idx)), n_arc + 1);
            theta_seq = theta_seq(1:end-1);
        else
            % 最后一段圆弧：保留终点
            theta_seq = linspace(deg2rad(arc_start(arc_idx)), deg2rad(arc_end(arc_idx)), n_arc);
        end
        for i = 1:length(theta_seq)
            if idx > n_points, break; end
...
function origins_all = generateLocalFrameOrigins(r_inner, arc_r, arc_cx, arc_cy, ...
    ...
    % ─────────────── 直线段：均匀分成 n_segments 份 ───────────────
    % 修复：原代码 linspace(-L_STRAIGHT/2, -L_STRAIGHT/2, ...) 两端点相同，
    % 导致所有直线段局部坐标系原点都集中在 y=-L_STRAIGHT/2，
    % 应为 linspace(-L_STRAIGHT/2, L_STRAIGHT/2, ...)
    ...
    for i = 1:n_angles
        ...
        for k = 1:n_frames
            ...
            R_z = [cos_t,  sin_t, 0;
                  -sin_t,  cos_t, 0;
                   0,      0,     1];
            points_local_frame(k, i, j, :) = (R_y * R_z * translated')';
            end
        end
    end
end
...
function [b_vals, alpha_vals, beta_vals] = generateTrapezoidParameters(L_straight, arc_r, arc_angle_rad, n_segments, d_half)
    ...
    % 直线段：每段的半长度 = L_straight / (2 * n_segments)
    % 修复：原代码 L_straight/4 是整段长度而非半长度，
    % 导致梯形棱柱体积为实际的两倍，产生重叠
...
function B_global = fullyInverseTransformMagneticField(B_local, local_angles_rad, rot_angles_y_rad)
    ...
    for k = 1:n_local_frames
        ...
        for i = 1:n_rot_angles
            ...
            cos_y = cos(theta_y);
            sin_y = sin(theta_y);
            R_y_inv = [cos_y,  0, sin_y;   % 绕Y轴旋转的逆矩阵（转置）
                       0,     1,  0;
                       -sin_y, 0,  cos_y];
            for j = 1:n_points
                B_local_vec = squeeze(B_local(k, i, j, :));
                B_intermediate = R_y_inv * B_local_vec;
                B_global_vec = R_z_inv * B_intermediate;
